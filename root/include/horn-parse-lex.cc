...
namespace YYNAMESPACE {
    ...
    extern gcstring yyprinted_location (Location_Type loc);
    extern int yylocation_line (Location_Type loc);
    extern gcstring yylocation_source (Location_Type loc);

    extern void yylex_init ();
    extern void yypush_lexer (FILE*, const gcstring&);
    extern void yypush_lexer (const gcstring&, const gcstring&);
...
#   endif
    ...
    public:
        ...
        /** The list of values I represent. */
        ...
        gcstring text () const {
            if (_text != NULL)
                return gcstring (_text, _text_size);
            if (_start+1 == _end) 
...
#ifdef YYSEMDUMPER
        void dump () const {
            fprintf (stderr, "<%d-%d: ", (int) _start, (int) _end);
    ...
    protected:
        ...
        typedef gcvector<ACTION_TAG> action_list;
        ...
        void set (list_type v, action_list* act,
         ...
         *  tree or list. */
        static action_list yyactstack;
        
    };
        
    yysembase::list_obj yysembase::yysemstack;
    yysembase::action_list yysembase::yyactstack;
    ...
    protected:
        ...
        static gcvector<SavedBinding> binding_stack;
        static gcvector<size_t> binding_fp;
    ...
    gcvector<yyvar::SavedBinding> yyvar::binding_stack;
    gcvector<size_t> yyvar::binding_fp;
...
static gcvector<Location_Type> yylast_locations;
gcvector< std::pair<yysem,int> > yypending_tokens;
...
static gcvector<gcstring*> input_sources;
static gcvector<gcstring> input_names;
...
/* of interactive version */
    ...
    static gcvector<size_t> free_link;
    static YYPARAM::list_obj tokens;
    static yysembase::action_list act;
};
#endif
...
gcvector<size_t> yylexlhs::free_link;
YYPARAM::list_obj yylexlhs::tokens;
yysembase::action_list yylexlhs::act;
...
yy_get_location (Location_Type loc, yy_size_t& s, int& line)
{
    ...
    line = 1;
    gcstring& input_source = *input_sources[s];
    for (size_t p = input_source.find ('\n'); 
         p != gcstring::npos && p+input_source.data () < loc; 
         p = input_source.find ('\n', p+1)) {
         line += 1;
    }
    return true;
...
yyprinted_location (Location_Type loc)
{
    ...
    result << line;
    return gcstring (result.str ().data (), result.str ().size());
}
...
yylexlhs::yyret ()
{
    if (token >= 0) {                                           
        ...
        if (!have_value)
            val = YYPARAM::default_make_token (token, _text_size, _text);
...
extern gcvector< std::pair<yysem,int> > yypending_tokens;
...
#ifdef YY_LEXER_2
void
yypush_lexer (FILE* inp, const gcstring& name)
{
...
void
yypush_lexer (const gcstring& inp, const gcstring& name)
{
    input_sources.push_back (new gcstring (inp));
    input_names.push_back (name);
    gcstring& input_source = *input_sources.back ();
